...
export const PASS_TEMPLATE_LIST = Object.values(PASS_TEMPLATES);

export function resolvePassTemplate(id?: string | null): PassTemplate {
  const key = (id || 'classic') as PassTemplateId;
  return PASS_TEMPLATES[key] ?? PASS_TEMPLATES.classic;
}

function isRgbOrHex(value?: string | null): value is string {
...
/** Never preview illegible Wallet colours (e.g. white stamp count on cream). */
...
/** Resolve cafe branding for live wallet preview in the owner app. */
export function resolveCafePassColors(cafe: {
  pass_template?: string | null;
  background_color?: string | null;
  foreground_color?: string | null;
  label_color?: string | null;
} | null | undefined): PassTemplate {
  const base = resolvePassTemplate(cafe?.pass_template);
  return {
    ...base,
    ...enforcePassContrast({
